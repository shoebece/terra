...
resource "azurerm_key_vault" "artifactory-kv" {
  name                       = "cdmz-artifactory-kv"
  resource_group_name        = azurerm_resource_group.artifactory-rg.name
  location                   = var.resource_location
  tenant_id                  = var.tenant_id
  sku_name                   = "standard"
  soft_delete_retention_days = 60
  ...
  depends_on = [azurerm_resource_group.artifactory-rg]
}

resource "azurerm_key_vault_key" "artifactory-disks-cmk" {
  name         = "cdmz-artifactory-disks-cmk"
  key_vault_id = azurerm_key_vault.artifactory-kv.id
  key_type     = "RSA"
  key_size     = 4096
  key_opts = [
    "decrypt",
  ...
  depends_on = [azurerm_key_vault.artifactory-kv]
}

resource "azurerm_key_vault_key" "artifactory-dbfs-cmk" {
